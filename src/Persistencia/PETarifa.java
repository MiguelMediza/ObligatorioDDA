...
public class PETarifa {
    ...
    public static Tarifa buscarTarifa(int pidTarifa) {
        ...
        try{
            ...
            if (var3.hasNext()) {
                ...
                try{
                    fechaVigencia = Utilidades.validarFechaTarifa(registro.get(2).toString());
                }
                catch(Exception e){
                    System.out.println(e.getMessage());
                }
    ...
    public static Tarifa buscarUltimaTarifa() {
        String sql = "SELECT * FROM tarifas WHERE fechaVigencia = (SELECT MAX(fechaVigencia) FROM tarifas);";
        try{
            Iterator var3 = conexion.seleccion(sql, null).iterator();
            if (var3.hasNext()) {
                List<Object> registro = (List) var3.next();
                int idTarifa = (Integer) registro.get(0);
                int monto = (Integer) (registro.get(1));
            ...
            System.out.println("Ocurrio un error al consultar la ultima tarifa");
            System.out.println(var6.getMessage());
            return null;
        }
    }
